...
function isAddCommand(text) {
  return /\b(add|save|submit|create|confirm|done)\b|जोड़ो|जोड़ दो|सेव करो|सेव|बनाओ|जमा करो|पक्का करो/.test(text);
}
...
function looksLikeProjectCommand(text, data) {
  ...
  const hasProjectSignal = /\bproject\b|प्रोजेक्ट|\bsite\b|साइट|\bbudget\b|बजट|\bstatus\b|स्टेटस/.test(text);
...
async function handleProjectSpeech(raw) {
  const text = normalize(raw);
  const data = extractProjectData(raw);
  ...
  const summary = `Project: ${document.getElementById("projectName").value || "-"} | Customer: ${document.getElementById("projectCustomer").value || "-"} | Site: ${document.getElementById("projectSite").value || "-"} | Budget: ${document.getElementById("projectBudget").value || "-"} | Status: ${document.getElementById("projectStatus").value || "-"}`;
  ...
  if (isAddCommand(text)) {
    ...
    if (nameVal) {
      ...
      if(success) showCommand("Project save ho gaya. " + summary);
    } else {
      showCommand("Project add karne ke liye project ka naam bolo.");
    }
  } else {
    showCommand("Project form bhara: " + summary + ". Bolo 'add karo' save karne ke liye.");
  }
  return true;
}
...
function handleButtonVoiceCommand(text) {
  const map = [
    ...
    { phrases: ["add project", "project save", "project jodo"], id: "addProjectBtn" },
...
async function handleSpeech(rawText) {
  ...
  try {
    ...
    // 2. Project (expense signal already excluded inside looksLikeProjectCommand)
    const projectData = extractProjectData(raw);
    if (looksLikeProjectCommand(text, projectData)) {
      await handleProjectSpeech(raw);
      return;
    }
...
function renderGalleryThumbs(photos) {
  ...
  photos.forEach((photo, idx) => {
    ...
    delBtn.style.cssText = "position:relative;margin-left:-24px;margin-top:-4px;background:#ef4444;color:#fff;border:none;border-radius:50%;width:20px;height:20px;font-size:11px;cursor:pointer;";
    ...
    img.addEventListener("click", () => {
      if (galleryMain) { galleryMain.src = galleryImageUrl(photo.fileId); galleryMain.style.display = "block"; }
    ...
    galleryThumbsBox.appendChild(wrap);
  });
...
(function () {
  ...
  const panelLoaders = {
    ...
    khataVoucherPanel: () => { loadLedgerDropdowns(); loadItemDropdown(); },
  ...
  async function loadLedgerDropdowns() {
    try {
      ...
      if (secSel) secSel.innerHTML = options;
  ...
  async function loadItemDropdown() {
    try {
      ...
      if (sel) sel.innerHTML = '<option value="">-- Select Item --</option>' +
        data.items.map(i => `<option value="${i._id}" data-rate="${i.sellingPrice}">${escapeHtml(i.itemName)} (Stock: ${i.stockQty})</option>`).join("");
  ...
  document.getElementById("saveVoucherBtn")?.addEventListener("click", async () => {
    ...
    if (!partyId) { alert("Ledger select karein."); return; }
    ...
    if ((voucherType === "Sales" || voucherType === "Purchase") && itemId) {
      ...
      const rate = parseFloat(itemSel.selectedOptions[0]?.dataset.rate) || 0;
      if (!amount) amount = rate * qty;
      items.push({ itemId, qty, rate });
    }

    if (!amount || amount <= 0) { alert("Amount daalein (ya item+qty se auto-calculate hone dein)."); return; }

    try {
      const res = await fetch(`${API_URL}/api/vouchers`, {
        method: "POST", headers: khataHeaders(),
        body: JSON.stringify({ voucherType, partyId, secondaryLedgerId, amount, items, note })
      });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || "Save fail hua");
